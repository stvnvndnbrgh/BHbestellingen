...
class KousDAO {
    public function getAll() {
        $sql = "select kous_id, merk, kwaliteit, drukklasse, lengte, maat, kleur, voetgrootte, bijzonderheden, bestelcode, barcode from kousen where leverancier_id = leverancier.id";
        $dbh = new PDO(DBConfig::$DB_CONNSTRING, DBConfig::$DB_USERNAME, DBConfig::$DB_PASSWORD);
        $resultSet = $dbh->query($sql);
        $lijst = array();
        foreach ($resultSet as $rij) {
            $leverancier = Leverancier::create($rij['leverancier_id'], $rij['leveranciernaam']); 
            $kous = Kous::create($rij['kous_id'], $rij['merk'], $rij['kwaliteit'], $rij['drukklasse'], $rij['lengte'], $rij['maat'], $rij['kleur'], $rij['voetgrootte'], $rij['bijzonderheden'], $rij['bestelcode'], $rij['barcode'],$leverancier);
